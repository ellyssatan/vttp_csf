package vttp_csf.day37server.controllers;
...
import vttp_csf.day37server.services.FileUploadService;
import vttp_csf.day37server.services.S3Service;
...
public class FileUploadController {
    ...
    public ResponseEntity<String> uploadForAngular(
        ...
        try {
            ...
            uploadSvc.uploadBlob(imgFile, title, complain);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public String retrieveImage(@PathVariable int postId, Model model) {
        
        Optional<Post> opt = uploadSvc.getPostById(postId);
        Post p = opt.get();
