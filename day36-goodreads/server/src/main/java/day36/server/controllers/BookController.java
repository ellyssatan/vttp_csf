package day36.server.controllers;
...
import day36.server.services.BookService;
...
public class BookController {
    ...
    public ResponseEntity<String> findBook(@PathVariable String bookId) {
        Optional<Book> opt = bSvc.findBook(bookId);
 
        if (opt.isEmpty()) {
            return ResponseEntity.status(404)
                .body(
                    Json.createObjectBuilder()
                        .add("error", "Cannot find book id %s".formatted((bookId)))
                        .build().toString()
                );
        }
